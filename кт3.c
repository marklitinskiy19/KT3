...
int main() {
    ...
    printf("Выполнил: Литинский Марк бИЦ-251\n\n");
...
// ВАША функция из формулы
double f(double x) {
    if (x < -1.0) {
        // Первая ветвь: f(x) = 1 - e^{-x}
        return 1.0 - exp(-x);
    }
    else if (x < 2.0) {
        // Вторая ветвь: f(x) = (x^2 + 1)/(x^2 - 1)
        // Обработка особой точки x = ±1
        if (fabs(x * x - 1.0) < 1e-10) {
            return 0.0;  // По определению f(1) = 0, f(-1) = 0
        }
        return (x * x + 1.0) / (x * x - 1.0);
    }
    else {
        // Третья ветвь: сумма от n=0 до 6 ((-1)^n * x^n)/(sqrt(n!) + 1)
        double sum = 0.0;
        for (int n = 0; n <= 6; n++) {
            double fact = factorial(n);
            sum += (pow(-1, n) * pow(x, n)) / (sqrt(fact) + 1.0);
        }
        return sum;
    }
}
...
int check_monotonic(double start, double end, double step) {
    ...
    if (step <= 0 || end <= start) {
        printf("Ошибка: Шаг должен быть положительным и начало интервала должно быть меньше конца интервала.\n");
        return 0;
    }
